$(document).ready(function () {
  $(".catalogoCard").on("click", "a#btnPedidoBook", function () {
    let data = $(this).data();
    let portada = "data:image/png;base64,";

    if (data.base64 && data.base64 !== 'None') {
        // Mostrar la imagen en base64
        $('#modal_catalogo #defult_in_portada').hide();
        $('#modal_catalogo #content_portada').attr('src', 'data:image/png;base64,' + data.base64).show();
    } else {
        // Restaurar imagen por defecto
        $('#modal_catalogo #content_portada').hide();
        $('#modal_catalogo #defult_in_portada').show();
    }
