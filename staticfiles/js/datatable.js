const datatable = (name_table, campo_order = 0, forma = 'desc', order = 0) => {
    let table = new DataTable('#' + name_table, {
        layout: {
            topEnd: {
                search: {
                    Text: false,
    ...
    // Función para controlar la información mostrada en las tablas
    // por periodos
    $('#periodo_opt').on('change', function (e) {
        let periodo = $(this).val();
        if (periodo != 'all') {
            $.ajax({
                url: '/inicio/get_periodo/',
                data: {"periodo": periodo},
                type: 'GET',
                success: function (response) {
                    fechas = [ response.fech_ini, response.fech_fin]
                    // Suponiendo que la respuesta devuelve un array de fechas ["2025-02-01", "2025-02-15"]
                    respuesta = generarFechasEnRango(fechas[0], fechas[1])
                    if (!respuesta.includes(fechas[1])) {
                        let partes = fechas[1].split("-"); // Divide "2024-12-01" en ["2024", "12", "01"]
                        respuesta.push(partes[2] + '-' + partes[1] + '-' + partes[0]);
                    }
                    // Filtrar en DataTables
                    table.column(order).search(respuesta.join('|'), true, false).draw();
                },
                error: function (error) { console.log(error); }
            });
        } else {
            // Mostrar todo si el usuario selecciona "Todo"
            table.column(order).search('').draw();
        }
...
function generarFechasEnRango(fechaInicio, fechaFin) {
    ...
    while (inicio <= fin) {  // Aseguramos que incluya la fecha final
        let dia = inicio.getDate().toString().padStart(2, '0');
        let mes = (inicio.getMonth() + 1).toString().padStart(2, '0'); // Meses van de 0 a 11
        let año = inicio.getFullYear();

        fechas.push(`${dia}-${mes}-${año}`);

        inicio.setDate(inicio.getDate() + 1); // Avanzamos un día
    }
